...
class NodeListTest {
    ...
    @Test
    void addEnd() {
        final NodeList<Name> nodeList = nodeList(new Name("abc"), new Name("bcd"), new Name("cde"));

        nodeList.addLast(new Name("xxx"));

        assertEquals(4, nodeList.size());
        assertEquals("[abc, bcd, cde, xxx]", nodeList.toString());
    }
